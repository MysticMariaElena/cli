...
import (
	...
	"github.com/cli/cli/internal/console"
...
func watchRun(opts *WatchOptions) error {
	...
	// TODO in a terminal emulator without ANSI this just prints repeatedly. it
	//      looks pretty shitty. going to experiment with cls.
	// TODO test on macos
...
func renderRun(opts WatchOptions, client *api.Client, repo ghrepo.Interface, run *shared.Run, prNumber string) (*shared.Run, error) {
	...
	if runtime.GOOS == "windows" {
		// Just clear whole screen; I wasn't able to get the same thing working
		//fmt.Fprint(opts.IO.Out, "\x1b[2J")
		fmt.Printf("\x1b[2J")
	} else {
		// Move cursor to 0,0
